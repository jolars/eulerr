...
arma::uvec set_intersect(arma::uvec x, arma::uvec y) {
  ...
  std::vector<int> a = conv_to< std::vector<int> >::from(x);
  std::vector<int> b = conv_to< std::vector<int> >::from(y);
  std::vector<int> out;
...
arma::vec return_intersections(arma::vec par, arma::vec areas, arma::umat id,
  ...
  arma::uvec itwos = find(twos == 1);
  arma::vec atwos = areas(find(twos == 1));

  arma::uvec ct = find(contained == 1);
  arma::uvec dj = find(disjoint == 1);
  arma::uvec is = find(intersecting == 1);
  ...
  arma::uvec isis = arma::find(join_cols(intersecting, intersecting) == 1);
  ...
  arma::umat all_circles = join_cols(two, two);
