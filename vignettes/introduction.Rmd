...
`fit2`. (Notice that these residuals are on the scale of the original values.)
However, the stress of the solution, which is the same statistic that 
Leland Wilkinson presented in his academic paper on venneuler 
@wilkinson_exact_2012, is low:

```{r stress}
fit2$stress
```

Now we can be confident in that the euler diagram we produce will be a
reasonable representation of our input. Were it otherwise, we should probably
...
Now that we know we can trust our fit, we can get to the fun part: plotting,
which is easy, and highly customizable, with eulerr
...
The default color palette is taken from
