...
performance, being that we're optimizing over 5 parameters for every ellipse
(or 3 in the case of circles) -- nevertheless, we're profitting immensely from
...
library \pkg{Armadillo}~\citep{eddelbuettel_2014} which ends up making the
code much faster than the java-based \pkg{venneuler}.
