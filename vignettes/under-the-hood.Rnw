...
to output. It is designed to an evolving documentation on the innards of the
program.

\section{Input}\label{input}

The workhorse of \pkg{eulerr} is \texttt{euler()}. To start with,
...
Our initial layout can be setup in a number of ways; \pkg{eulerr}
...
Wilkinson's R package
\href{https://CRAN.R-project.org/package=venneuler}{venneuler}~\citet{wilkinson_2012}.
\pkg{venneuler} tries to place disjoint and subset
...
The MDS algorithm from \pkg{venn.js} circumvents this by assigning a
...
pairwise relationsships between sets, \pkg{eulerr} uses the following
loss, gradient, and Hessian functions:
\begin{fullwidth}
\begin{equation}
...
swapped for $k_i$ (and vice versa), $F$ are the various sets in the
input, $\Omega$ are the pairwise overlaps between circles. Because it speeds up
convergence, we also
...
Fredrickson uses the \emph{Polak--Ribiére Conjugate Gradient Method} to
optimize the initial layout. In our experience, this method occasionally
encounters local minima, which is why we have opted to use
\texttt{nlminb()} from the R core package \pkg{stats}, which is
a translation from \proglang{FORTRAN} code developed by~\citet{gray_1990}
and ported to R by Douglas Bates and Deepayan Sarkar, and,
although it is a piece of complicated code, performs well for the
difficult problem of aligning Euler diagrams.
...
by, for instance, quad-tree binning (\pkg{venneuler}) or polygon
intersections (\pkg{VennMaster}~\citep{kestler_2008}). These methods
...
\pkg{venn.js} and \pkg{eulerAPE} both, however, use exact
...
the points at which the different ellipses intersect. \pkg{eulerr}'s
approach to this is based on a method outlined by \citep{richter-gebert_2011}.
\pkg{eulerr} owes significant debt to the R package
\pkg{RConics}~\citep{huber_2014}, which has been tremendously helpful in
developing and, especially, debugging the algorithm. Some parts of the
code are in fact straight-up translations to \proglang{C++} from the code in
\pkg{RConics}.
...
possible combinations of ellipses. A solution for this, albeit only for circles,
was first published by Fredrickson in a
...
Here, we have used an algorithm from~\citet{eberly_2016}, which computes
...
As our loss function, we use the sum of squared differences between the disjoint
set intersections and the areas we have computed and again use the
\texttt{nlm()} optimizer to layout the set.

In \emph{rare} instances, the algorithm breaks down because of numerical precision
errors that might, for instance, lead to some intersection points being
left out. In these cases, we rely instead on a approximation by sampling
points inside the ellipses and comptuting the area of the required overlap
as the proportion of points inside that overlap to those in the ellipse
multiplied with the ellipse's area.

This optimization step is the bottleneck of the overall computations in terms of
performance, being that we're optimizing over five parameters for every ellipse
(or 3 in the case of circles)---nevertheless, we're profitting from
the implementation in the \proglang{C++} programming language through
\pkg{Rcpp}~\citep{eddelbuettel_2011} and its plugin for the linear algebra
library \pkg{Armadillo}~\citep{eddelbuettel_2014}. When the number of sets
is low, \pkg{eulerr} outperforms \pkg{venneuler}, whereas the
relationship reverses as the number of sets increase (around seven), when
the sheer number of overlaps that \pkg{eulerr} has to examine bogs it down.

In a future version, it is possible that a alternative, approximative method
will be introduced to deal with relationships with large numbers of sets.

\section{Layout}\label{layout}

Since the optimization steps are unconstrained, we run the risk of
ending up with dispersed layouts. To fix this, we use a SKYLINE-BL
rectangle packing algorithm~\citep{jylanki_2010} to pack the disjoint clusters
...
the fit from \pkg{eulerr} matches the input. Sometimes euler diagrams
...
well''. \pkg{venneuler} uses a metric called \emph{stress}, which is
defined as \[
...
Meanwhile, \textbf{eulerAPE}~\citep{micallef_2014} uses
\emph{diagError}: \[
...
\textbf{eulerr} interfaces the elegant Lattice graphics
system~\citep{sarkar_2008} to grant the user extensive control over the output,
and allow for facetted plots in case such a design was used in fitting the Euler
configuration.
...
<<quantities-plot, eval = FALSE>>=
plot(euler(c(A = 2, B = 3, "A&B" = 1)), quantities = TRUE)
@

\begin{marginfigure}
<<quantities-dev, ref.label = "quantities-plot", echo = FALSE>>=
@
\caption{A plot with quantities.}
\label{fig:quantities-plot}
\end{marginfigure}
...
the point inside the circle overlap for the quantities and circle complement
...
points uniformly, we use \emph{Vogel's method}~\citep{arthur_2015,vogel_1979}
\[
...
\emph{Nelder--Mead Method}~\citep{nelder_1965} which we've translated
from Matlab code by~\citet{kelley_1999} and customized for \pkg{eulerr} (in
