...
argument in `euler()`, which mirrors the loss functions provided by the
underlying `eunoia` Rust crate.
...
diagError                  `diag_error`                     $\max_i \big|y_i/\sum_k y_k - \hat y_i / \sum_k \hat y_k\big|$ [@Micallef_2014a]
...
looked at, we now refit the diagram for each loss.

```{r fig.cap = "Euler diagrams fit to the combination above, using different loss functions", fig.show = "hold", fig.width = 3, fig.height = 3}
losses <- c(
  "sum_squared",
  "sum_absolute",
