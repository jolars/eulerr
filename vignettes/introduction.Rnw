...
\emph{venneuler}, on the other hand, will take any number of sets (in theory),
...
\begin{figure}
\sidecaption{A difficult combination modeled with ellipses.\label{fig:wilkinson2012}}
...
parameters~(\cref{fig:eulerr-plot}) can easily be adjusted to suit anybody's
...
excellent facilities made available by the core R package \pkg{grid}.
