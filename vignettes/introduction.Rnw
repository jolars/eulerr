...
@
\caption{Residuals for the fit diagram.}
...
and the \emph{diagError} statistic from \pkg{eulerAPE} \cite{micallef2014}:
...
\href{https://CRAN.R-project.org/package=lattice}{lattice} and interfaces all of
its bells and whistles to allow for extensive customization of the resulting
plots.

\pkg{eulerr}'s default color palette is taken from
\href{https://cran.r-project.org/package=qualpalr}{qualpalr}---another package
that I have developed---which uses color difference algorithms to generate
distinct qualitative color palettes.
