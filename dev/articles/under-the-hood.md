...
$`i`$^(th) and $`j`$^(th) sets respectively, $`O_{ij}`$ their overlap,
and $`d_{ij}`$ their separation.
...
Setting $`r_i = \sqrt{F_i/\pi}`$, where $`F_i`$ is the size of the
$`i`$^(th) set, we are able to obtain $`d`$ numerically using the
squared difference between $`O`$ and the desired overlap as loss
function,
...
[`optimize()`](https://rdrr.io/r/stats/optimize.html)[^1]
...
place the circles arbitrarily as long as their separation, $`d`$,
...
current release of R suffers from a bug[^2] causing the analytical
...
of these intersections, for which we will use the shorthand $`\omega`$.
We introduced this form in the *input* section, but now define it
rigorously.

For a family of *N* sets, $`F = F_1, F_2, \dots, F_N`$, and their
$`n=2^N-1`$ intersections, we define $`\omega`$ as the intersections of
...
$`A`$, so that an exact diagram requires that $`\omega_i = A_i`$ for
$`i=1,2,\dots,2^N-1`$, where $`N`$ is the number of sets in the input.
...
to the semi-major axis $`a`$, $`b`$ is the semi-minor axis, and $`h, k`$
are the x- and y-coordinates, respectively, of ellipse’s center.
...
However, because an ellipse is a conic[^3] it can be represented in
quadric form,
...
3.  integrating the ellipse over \[$`0,\theta_0`$\] and
    \[$`0,\theta_1`$\], producing elliptical sectors $`F(\theta_0)`$ and
    $`F(\theta_1)`$,
4.  subtracting the smaller ($`F(\theta_0`$)) of these sectors from the
    larger ($`F(\theta_0`$), and
...
retrieve the equivalent of $`(A \cap B) \setminus C`$.

The exact algorithm may in rare instances[^4], break down, the culprit
being numerical precision issues that occur when ellipses are tangent or
completely overlap. In these cases, the algorithm will approximate the
area of the involved overlap by
...
intersections and their relative complements, $`\omega`$, up to
numerical precision.
...
Eberly, David. 2016. “The Area of Intersecting Ellipses.” In *Geometric
Tools*.
...
Frederickson, Ben. 2016. *Venn.js Area Proportional Venn and Euler
...
Guided Tour Through Real and Complex Geometry*. 1st ed. Springer.
...
[^5]: We conducted thorough benchmarking, that we opt not to report
    here, to decide upon an algorithm for this step.
