...
- `fills`, `patterns`, `edges`, `labels`, and `quantities` in
  [`plot.euler()`](https://jolars.github.io/eulerr/dev/reference/plot.euler.md)
  now accept a `by_group` entry for per-panel styling overrides on
  diagrams fit with `by =`. Keys are panel names (the names of the
  fitted object, e.g. `"male"` or `"male.adult"` for multi-`by`); panels
